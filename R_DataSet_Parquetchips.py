...
def write_trading_daily_report_outputs(final_df: pd.DataFrame, exec_ts: str, keep_history: bool, mode: str) -> dict:
    ...
    # TradingDailyReport 銝?蝝舐?憭瑼????撟游漲瘣餅? + ?芣迫?餃僑撠?瑼?
    # ?園? dataset ?風?脫?靽?閬?銝?敶梢??
...
def fetch_dataset_rows(
    ...
    try:
        ...
        if rows:
            print(
                f"{dataset_name}: all-market query with data_id=???returned rows", flush=True)
            return rows
        print(
            f"{dataset_name}: all-market query with data_id=???returned no rows", flush=True)
    except Exception as exc:
        errors.append(str(exc))
        print(f"{dataset_name}: all-market query with data_id=???failed, continue fallback. reason={exc}", flush=True)
